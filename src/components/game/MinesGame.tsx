    ...
    <div className="rounded-2xl border border-white/10 p-4">
      ...
      {!live ? (
        <div className="mt-4 border-t border-white/10 pt-3">
          <p className="mb-2 text-[11px] font-medium tracking-wide text-amber-200/90">No limit — go all-in if you want. Your whole cash is on the table.</p>
          <div className="flex flex-wrap items-end gap-3">
            <div>
              ...
              <input
                ...
                max={cash}
                value={stake}
                onChange={(e) => setStake(Math.max(1, Math.min(cash, Number(e.target.value)||1)))}
                className="mt-1 w-32 rounded-xl border border-[var(--line)] bg-[#0b0e14] px-3 py-2 text-sm text-white outline-none transition focus:border-amber-200/40"
              />
            </div>
            <div className="flex flex-wrap gap-1">
              {[0.5, 2].map((m) => (
                <button
                  key={m}
                  onClick={() => setStake(Math.max(1, Math.round(stake * m)))}
                  className="rounded-full border border-white/10 px-2.5 py-1 text-xs text-[var(--muted)] hover:border-amber-200/30 hover:text-amber-200 transition"
                >
                  {m}×
                </button>
              ))}
              <button
                onClick={() => setStake(Math.max(1, Math.round(cash * 0.5)))}
                className="rounded-full border border-white/10 px-2.5 py-1 text-xs text-[var(--muted)] hover:border-amber-200/30 hover:text-amber-200 transition"
              >
                ½ cash
              </button>
              <button
                ...
                className="rounded-full border border-amber-300/50 bg-amber-300/10 px-3 py-1 text-xs font-semibold text-amber-200 hover:bg-amber-300/20 transition"
              >
                ALL IN · {formatINR(cash)}
              </button>
            </div>
            <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <Btn disabled={busy || stake <= 0 || stake > cash} onClick={() => void act({ type: "minesStart", stake, mines })}>
              {stake===cash && cash>0 ? `Go ALL IN ${formatINR(stake)}` : "Start round"}
            </Btn>
            <span className="text-xs text-[var(--muted)]">
              Liquid cash {formatINR(cash)} · {stake===cash? "ALL IN — no limit, whole stack at risk" : stake>cash/2? "big bet": "no cap"}
